...
const shouldRecreateStore = (store: SplitSubvariantStore, props: SplitSubvariantProps) => {
	const { state } = store.getState()
	return state !== props.state
}

export function SplitSubvariantStoreProvider({ children, ...props }: SplitSubvariantProviderProps) {
...
export function useSplitSubvariantStore<T>(selector: (state: SplitSubvariantState) => T): T {
	const useStore = useContext(SplitSubvariantStoreContext)
	if (!useStore) {
		throw new Error(`You did not wrap your component in <${SplitSubvariantStoreProvider.name}>.`)
	}
