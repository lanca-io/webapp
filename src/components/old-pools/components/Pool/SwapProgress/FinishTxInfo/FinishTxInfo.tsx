...
export const FinishTxInfo = memo(
	({ to: { amount, token, chain }, isDeposit }: Props) => {
		const title = isDeposit ? 'Deposited' : 'Withdrawal'
		const alertTitle = isDeposit
			? 'Withdrawal will be available in 7 days'
			: 'Your funds will arrive in 7 days'
		const alertSubtitle = isDeposit
			? ''
			: `Track its progress in 'Actions History'`
		return (
