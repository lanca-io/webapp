...
export const ModeMenu = memo((): JSX.Element => {
	...
	const handleSwapClick = useCallback(
		() => setState(SplitSubvariantType.SWAP),
		[setState],
	)
	const handleSendClick = useCallback(
		() => setState(SplitSubvariantType.SEND),
		[setState],
	)

	return (
		<div
			className="mode-menu-container"
			role="group"
			aria-label="Transaction mode selection"
		>
			<ModeButton
				isActive={state === SplitSubvariantType.SWAP}
				onClick={handleSwapClick}
				text="Swap"
			/>
			<ModeButton
				isActive={state === SplitSubvariantType.SEND}
