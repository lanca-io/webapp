...
export const DestinationPanel = memo(
	({ amount, isLoading }: DestinationPanelProps): JSX.Element => {
		...
		const hasBalance = Boolean(
			toToken?.balance !== undefined && toToken?.balance !== null,
		)
		...
		return (
			<div className="destination_panel">
				<div className="destination_panel_input">
					...
					<DestinationValue />
					{address && hasBalance && (
						<BalanceInfo token={toToken} showMax={false} />
					)}
				</div>
				...
				{routeLoading ? (
					...
					<Alert
						title={error}
						variant="error"
						icon={<InfoIcon color="var(--color-danger-600)" />}
					/>
				) : route ? (
